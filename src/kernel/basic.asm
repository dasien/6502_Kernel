...
Ram_top           = $8800     ; end of user RAM+1 (before the always-mapped DOS ROM at $8800-$AFFF). The cold-start RAM probe also stops here on its own, since $8800 is ROM and fails the write/read-back test.
...
LAB_295E:
      ...
      ; Entry: A = high byte, X = low byte of 16-bit unsigned integer
      ; Exit: Null-terminated string in Decssp1, prints via LAB_18C3
      ; Uses: FAC1_1, FAC1_2, FAC1_3, hardware stack

      STA   FAC1_1            ; save high byte
      STX   FAC1_2            ; save low byte

      ; Check for zero
      ORA   FAC1_2            ; check if both bytes are zero
      BNE   LAB_295E_NOT_ZERO ; branch if not zero

      ; Handle zero: output "0"
      LDA   #'0'              ; load ASCII '0'
      STA   Decssp1           ; store to string buffer
      LDA   #$00              ; null terminator
      STA   Decssp1+1         ; terminate string
      LDA   #<Decssp1         ; load string pointer low byte
      LDY   #>Decssp1         ; load string pointer high byte
      JMP   LAB_18C3          ; print and return

LAB_295E_NOT_ZERO:
      ; Extract decimal digits by repeatedly dividing by 10
      ; Push digits onto hardware stack in reverse order
      LDY   #$00              ; initialize digit counter

LAB_295E_DIV_LOOP:
      ; Check if value is zero
      LDA   FAC1_1            ; load high byte
      ORA   FAC1_2            ; OR with low byte
      BEQ   LAB_295E_DONE_DIV ; branch if zero (no more digits)

      ; Divide by 10, remainder in A
      JSR   LAB_295E_DIV10    ; divide FAC1_1:FAC1_2 by 10

      ; Convert remainder (0-9) to ASCII and push onto stack
      CLC                     ; clear carry
      ADC   #'0'              ; convert to ASCII ('0' = $30)
      PHA                     ; push digit onto stack
      INY                     ; increment digit count
      BNE   LAB_295E_DIV_LOOP ; continue (max 5 digits, Y won't overflow)

LAB_295E_DONE_DIV:
      ; Pop digits from stack and build string in correct order
      LDX   #$00              ; initialize string index

LAB_295E_POP_LOOP:
      CPY   #$00              ; check digit count
      BEQ   LAB_295E_POP_DONE ; branch if no more digits
      PLA                     ; pop digit from stack
      STA   Decssp1,X         ; store to string buffer
      INX                     ; increment string index
      DEY                     ; decrement digit count
      BNE   LAB_295E_POP_LOOP ; continue popping

LAB_295E_POP_DONE:
      ; Null-terminate string
      LDA   #$00              ; null terminator
      STA   Decssp1,X         ; terminate string

      ; Return string pointer and print
      LDA   #<Decssp1         ; load string pointer low byte
      LDY   #>Decssp1         ; load string pointer high byte
      JMP   LAB_18C3          ; print and return

; -------------------------------------------------------------------------
; Divide 16-bit unsigned integer by 10
; Entry: FAC1_1:FAC1_2 = dividend (high:low)
; Exit: FAC1_1:FAC1_2 = quotient, A = remainder (0-9)
; Uses: FAC1_3 (remainder accumulator), X (bit counter)
; -------------------------------------------------------------------------
LAB_295E_DIV10:
      LDA   #$00              ; clear A
      STA   FAC1_3            ; clear remainder
      LDX   #$10              ; 16 bits to process

LAB_295E_DIV10_LOOP:
      ASL   FAC1_2            ; shift dividend left (low byte)
      ROL   FAC1_1            ; shift dividend left (high byte)
      ROL   FAC1_3            ; shift bit into remainder

      ; Check if remainder >= 10
      LDA   FAC1_3            ; load remainder
      CMP   #$0A              ; compare with 10
      BCC   LAB_295E_DIV10_NEXT ; branch if remainder < 10

      ; Subtract 10 from remainder
      SBC   #$0A              ; subtract 10 (carry already set from CMP)
      STA   FAC1_3            ; store new remainder

      ; Set bit in quotient
      INC   FAC1_2            ; set bit 0 (we just shifted, so it's clear)

LAB_295E_DIV10_NEXT:
      DEX                     ; decrement bit counter
      BNE   LAB_295E_DIV10_LOOP ; continue if more bits

      LDA   FAC1_3            ; return remainder in A
      RTS
