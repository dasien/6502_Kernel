...
bool VIC::isScreenAddress(uint16_t address) const
{
    return address >= kScreenMemoryStart && address <= kScreenMemoryEnd;
}

void VIC::writeScreen(uint16_t address, uint8_t value)
{
    ...
    uint16_t offset = addressToOffset(address);
...
uint8_t VIC::readScreen(uint16_t address) const
{
    if (!isScreenAddress(address))
    {
        return 0x00;
...
uint8_t VIC::getCharacterAt(uint16_t x, uint16_t y) const
{
    ...
    uint16_t offset = coordinatesToOffset(x, y);
    return screen_buffer_[offset];
}

void VIC::setCharacterAt(uint16_t x, uint16_t y, uint8_t character)
{
    if (x >= kScreenWidth || y >= kScreenHeight)
    {
        return;
...
void VIC::clearScreen(uint8_t fill_char)
{
...
void VIC::scrollUp()
{
    ...
    for (uint16_t y = 0; y < kScreenHeight - 1; ++y)
    {
        ...
        {
            uint16_t current_offset = coordinatesToOffset(x, y);
            uint16_t next_line_offset = coordinatesToOffset(x, y + 1);
    ...
    for (uint16_t x = 0; x < kScreenWidth; ++x)
    {
        uint16_t bottom_offset = coordinatesToOffset(x, kScreenHeight - 1);
...
void VIC::setCursorPosition(uint16_t x, uint16_t y)
{
    if (x < kScreenWidth && y < kScreenHeight)
    {
        cursor_x_ = x;
...
uint16_t VIC::addressToOffset(uint16_t address) const
{
    return address - kScreenMemoryStart;
}

uint16_t VIC::coordinatesToOffset(uint16_t x, uint16_t y) const
{
    return y * kScreenWidth + x;
}

void VIC::offsetToCoordinates(uint16_t offset, uint16_t& x, uint16_t& y) const
{
