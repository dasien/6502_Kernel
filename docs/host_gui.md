...
plane, sprites and cursor are rendered, and how host input reaches the machine. The
virtual chipset behind all of it is described in [board.md](board.md).
...
2. Click the 6502 screen area to give it focus.
3. Type monitor commands straight into it, such as `W:8000 FF` or `R:8000`.
4. Watch the CPU status update live in the sidebar as it runs.
5. The result behaves as a genuine 6502 terminal rather than as a console app.
