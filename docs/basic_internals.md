# MFC BASIC Label Glossary

A label map for the EhBASIC-derived interpreter. Upstream EhBASIC is unmaintained
and its labels are terse, so this glossary is what makes the source navigable
rather than renaming them and diverging from upstream. The BASIC user manual is
[BASIC.md](BASIC.md).

---

A reference for the cryptic `LAB_<hex>` labels in `src/kernel/basic.asm`. MFC BASIC,
the interpreter, is derived from EhBASIC, which the `NOTICE` file credits. Renaming
around 780 code labels in place would have been a large and error-prone change, so the
