...
| `$FE00-$FED0` | Memory-mapped I/O (inside the kernel window, reserved by the `IORESV` linker segment) |
...
- **`$FE00-$FED0`**: Memory-mapped I/O — PIA (incl. the live held-key port at `$FE0F`),
  `MODULE_BANK`, BlockDevice, ACIA, VIC register port, SID, RTC, PowerSwitch, VIC
  soft-font port, VIC sprite block, the VIC frame counter and the sprite pattern port. The 80x25 screen and its colours live behind the
  VIC port, not in the 64K map. **First free byte: `$FED1`** (47 left).
